...
Version 1.0.2 is a focused UI hardening release. It fixes text clipping and overlapping labels observed on real Windows while preserving the Detector v3, expanded RU/EN lexicons and runtime behavior from 1.0.1.
...
- OEM-key candidate tracking supports words whose opposite-layout form begins with punctuation-looking keys, including `,kz` → `бля`, while retaining `rjhj,rf` → `коробка` and `cdj,jle` → `свободу` behavior.
