...
fn wikipedia_detector_corpus() {
    ...
    let report_path = PathBuf::from(
        env::var("G_SWITCHER_DETECTOR_REPORT")
            .unwrap_or_else(|_| "target/wiki-detector-failures.csv".into()),
    );
    let summary_path = PathBuf::from(
        env::var("G_SWITCHER_DETECTOR_SUMMARY")
            .unwrap_or_else(|_| "target/wiki-detector-summary.txt".into()),
    );
    ...
    let mut csv = String::from(
        "occurrences,language,expected,synthetic,actual,confidence,previous_context\n",
    );
    ...
    eprintln!(
        "detector corpus occurrences={} variants={} failed={}",
        occurrences.len(),
        failures.len(),
        failed_occurrences
    );
...
fn parse_occurrences(text: &str) -> Vec<Occurrence> {
    ...
    let flush = |result: &mut Vec<Occurrence>,
                 current: &mut String,
    ...
    for ch in text.chars().chain(std::iter::once(' ')) {
        ...
        match (language, next) {
            ...
            (Some(_), Some(n)) => {
                flush(&mut result, &mut current, &mut language, &mut context);
                language = Some(n);
                current.push(ch);
            }
            (Some(_), None) => {
