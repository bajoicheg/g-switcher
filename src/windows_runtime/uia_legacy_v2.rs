...
pub fn has_adapter(hwnd: HWND) -> bool {
    ...
    text_of(&document).is_some() && selection_offsets(&context.text, &document).is_some()
}
...
/// Verified whole-value mutation for Chromium/WebView2/Electron text controls
/// that expose TextPattern plus LegacyIAccessible. The mutation is fail-open:
/// before SetValue a fresh UIA context must still have the same RuntimeId,
/// exact document text, and exact selection/caret snapshot that was observed
/// while planning the range replacement. Unlike the previous implementation,
/// this preflight never selects the target range, because Select() itself can
/// perturb Chromium provider state and make a safe Undo nondeterministic.
/// Post-state and caret are verified on freshly reacquired provider objects.
pub fn replace_range_if_matches(
    ...
    // Reacquire immediately before mutation. This catches focus, element,
    // document and caret/selection races without changing the target state.
    let Some(context) = focused_context(hwnd) else {
        trace("lost legacy context before mutation");
        return false;
    };
    if context.runtime_id != original_id {
        trace("RuntimeId changed before mutation");
        return false;
    }
    let Some(recheck_document) = (unsafe { context.text.DocumentRange().ok() }) else {
        trace("cannot reacquire document before mutation");
        return false;
    };
    if text_of(&recheck_document).as_deref() != Some(before.as_str())
        || selection_offsets(&context.text, &recheck_document) != Some(original_selection)
    {
        trace("fresh pre-mutation state changed");
        return false;
    }
...
fn focused_context(hwnd: HWND) -> Option<LegacyContext> {
    ...
    AUTOMATION.with(|slot| {
        ...
        // An explicit ValuePattern read-only declaration always wins.
...
fn trace(message: &str) {
    eprintln!("G-switcher LegacyIAccessible v2 adapter: {message}");
}
